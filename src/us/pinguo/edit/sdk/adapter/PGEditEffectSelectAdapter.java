...
public class PGEditEffectSelectAdapter extends BaseHoriScrollItemAdapter {
    ...
    public View initView(final LinearHoriScrollView parent, Context context, final int position) {
        ...
        View item;
        item = View.inflate(context, EUExUtil.getResLayoutID("plugin_uexcamera360_layout_effect_select_item"), null);
        ImageLoaderView icon = (ImageLoaderView) item.findViewById(EUExUtil.getResIdID("effect_image"));
        icon.setScaleType(ImageView.ScaleType.FIT_XY);
        icon.setImageUrl(effect.getIconFileUrl(context));
        TextView text = (TextView) item.findViewById(EUExUtil.getResIdID("effect_text"));
        text.setBackgroundColor(effect.getColor());
        String info = SystemUtils.getLocationInfo();
        text.setText(effect.getName(info.replace("-", "_")));
        ImageView ivMask = (ImageView) item.findViewById(EUExUtil.getResIdID("effect_mask"));
        ivMask.setBackgroundColor(effect.getColor() & 0xb3ffffff);
        View effectStateView = item.findViewById(EUExUtil.getResIdID("effect_state_parent"));
        View view = item.findViewById(EUExUtil.getResIdID("id_effect_click_state"));
        ...
        View imageClick = item.findViewById(EUExUtil.getResIdID("effect_image_container"));
    ...
    public void changeSelectPosition(int position) {
        ...
        View lastStateView = findEffectStateView(lastSelected, EUExUtil.getResIdID("effect_state_parent"));
        View lastClickStateView = findEffectStateView(lastSelected, EUExUtil.getResIdID("id_effect_click_state"));
        ...
        View stateView = findEffectStateView(position, EUExUtil.getResIdID("effect_state_parent"));
        View clickStateView = findEffectStateView(position, EUExUtil.getResIdID("id_effect_click_state"));
